...
import { pilgrimSatisfactionData } from "../../data/reportData";
...
export function PilgrimSatisfactionChart() {
  ...
  const option: EChartsOption = {
    ...
    yAxis: {
      type: "category",
      data: pilgrimSatisfactionData.map((item) => item.axis),
      inverse: true,
      axisTick: {
        show: false,
      },
    ...
    series: [
      {
        name: "نسبة الرضا",
        type: "bar",
        data: pilgrimSatisfactionData.map((item) => item.satisfaction),
        barWidth: 18,
        label: {
          show: true,
          position: "right",
    ...
    <article ref={elementRef} className="glass-card rounded-[2rem] p-5 sm:p-7">
      <div className="mb-6">
        ...
        <h3 className="text-2xl font-black text-white sm:text-3xl">
          مؤشرات الرضا حسب المحاور
        </h3>

        <p className="mt-3 max-w-3xl leading-7 text-slate-300">
          يوضح الرسم أعلى وأقل محاور الرضا، مع ظهور خدمات مزدلفة كأكبر محور
          يحتاج إلى تحسين مباشر.
        </p>
      </div>

      <div className="chart-shell h-[470px] max-w-full overflow-hidden rounded-3xl border border-white/10 bg-slate-950/50 p-3 sm:h-[430px]">
        {hasEntered ? (
          <ReactECharts
            option={option}
            notMerge
