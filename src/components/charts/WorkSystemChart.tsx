...
import { workSystemData } from "../../data/reportData";
...
export function WorkSystemChart() {
  ...
  const option: EChartsOption = {
    ...
    yAxis: {
      type: "category",
      data: workSystemData.map((item) => item.axis),
      inverse: true,
      axisTick: {
        show: false,
      },
    ...
    series: [
      {
        name: "نسبة الرضا",
        type: "bar",
        data: workSystemData.map((item) => item.satisfaction),
        barWidth: 15,
        label: {
          show: true,
          position: "right",
    ...
    <article ref={elementRef} className="glass-card rounded-[2rem] p-5 sm:p-7">
      <div className="mb-6">
        ...
        <h3 className="text-2xl font-black text-white">
          تقييم المحاور التشغيلية
        </h3>

        <p className="mt-3 leading-7 text-slate-300">
          يوضح تقييم مشرفي المجموعات لمحاور المنظومة، مع ظهور تطبيق الحملة
          كأقل محور.
        </p>
      </div>

      <div className="chart-shell h-[540px] max-w-full overflow-hidden rounded-3xl border border-white/10 bg-slate-950/50 p-3 sm:h-[430px]">
        {hasEntered ? (
          <ReactECharts
            option={option}
            notMerge
